...
- XML에서 미디어 URL 추출 (다중 경로 지원):
  1. `[MEDIA_FILE]` 템플릿 치환
  2. 직접 `.mp4` URL
  3. `desktop.html5.media_uri` 경로
  4. `content_uri` 기반 fallback 조합

### 4-2. 개별 다운로드 (`download-video`)
- 파일 저장 다이얼로그로 경로 선택
- MP4 또는 MP3 포맷 선택 가능
- 파일명 특수문자 자동 치환 (`/\?%*:|"<>` → `_`)

### 4-3. 일괄 다운로드 (`download-all`)
- 폴더 선택 다이얼로그
- 최대 **3개** 동시 다운로드 (worker pool 패턴)
- 개별 성공/실패 결과 반환
...
- 3xx 리다이렉트 자동 추적
- 타임아웃: **5분** (300,000ms)
- 실시간 진행률 이벤트 (`download-progress`)

## 5. 오디오 처리

### 5-1. MP4 → MP3 변환
- FFmpeg `libmp3lame` 코덱, 오디오 품질 2
...
### 6-1. Gemini API 키 관리
- **저장**: `safeStorage.encryptString()` → `userData/gemini-key.enc`
- **로드**: `safeStorage.decryptString()`
- **삭제**: 파일 내용을 빈 문자열로 덮어쓰기

### 6-2. 단일 파일 변환 (`transcribe-audio`)
- MP3 파일을 base64 인코딩하여 Gemini API에 전송
...
### 6-3. 일괄 변환 (`transcribe-batch`)
- 지정 폴더의 모든 MP3 파일 그룹핑 (`groupMp3Files`)
- 분할 파일은 같은 그룹으로 묶음
- 최대 **2개** 동시 변환 (worker pool 패턴)

### 6-4. 다운로드 + 변환 통합 (`download-and-transcribe-all`)
- 1단계: 전체 영상 MP3 다운로드 (최대 3개 동시)
- 2단계: 다운로드된 MP3 그룹별 텍스트 변환 (최대 2개 동시)
- 모든 다운로드 실패 시 2단계 스킵

### 6-5. 에러 처리
- **Rate limit (429)**: 지수 백오프 재시도 (2초 → 4초 → 8초, 최대 3회)
...
| **asar 호환** | FFmpeg 바이너리 경로에서 `app.asar` → `app.asar.unpacked` 보정 |
| **앱 종료** | LMS 창 명시적 파괴 (`before-quit` 이벤트), macOS는 창 전체 닫혀도 앱 유지 |
